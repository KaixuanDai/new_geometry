...
function judge(id) {
    ...
    if (pickinfo.hit) {
        ...
        if(pickMesh.state.toString().slice(5,)==inMesh.state)
        {
            ...
            var music = new BABYLON.Sound("music", "/../audio/define_box_win.mp3",scene, null, { loop: false, autoplay: true, spatialSound: true });
...
function pagesLink() {
    ...
    var new_url = '/html/level/level_define_rec/level_define_rec.html';
...
function afterJudge(id) {
    ...
    var m = scene.getMeshByID("true_box");
