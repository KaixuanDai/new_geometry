...
function judge(id) {
    ...
    if (pickinfo.hit) {
        ...
        if(pickMesh.state.toString().slice(5,)==inMesh.state)
        {
            ...
            var music = new BABYLON.Sound("music", "/../audio/define_tri_win.mp3",scene, null, { loop: false, autoplay: true, spatialSound: true });
...
function afterJudge(id) {
    ...
    var i = id;
    var m = scene.getMeshByID("true_tri");
    m.state="false";
